...
--收费
select mac,out_trade_no,add_time,FROM_UNIXTIME(add_time,'%Y-%m-%d %H:%i:%S'),status from tb_orders where  total_price>0 and out_trade_no > '201910021731' and out_trade_no < '201910031731' order by id desc;
...
--播放记录  
select t.name_remark as grade, t.name as point_name,v.name,count(v.video_id) as total,v.created_at from tb_course_structure_tree as t 
  inner join tb_course_video_detail as c 
  on t.id =c.foreign_course_structure_tree
  inner join tb_video_report as v on v.video_id = c.id where v.created_at < '2019-10-07 17:31'   group by v.name having total >0 order by total desc;
  

select a.channel,a.mac,a.zone,a.device,a.created_at,a.updated_at, (case when a.mac in (
    ...
    ) then '是' else '否' end ) as repeat_stat from tb_launch_log as a
 where a.created_at < '2019-10-07 17:31' order by a.created_at desc;
...
select * from tb_launch_log where created_at < '2019-10-01 17:31' and created_at > '2019-09-30 17:31' and mac in (
select mac from tb_member_uuid where created_at < '2019-09-24 17:31' and created_at > '2019-09-23 17:31'
);
...
--浏览记录
select * from tb_visitor_history where url like 'http://210.14.68.73:20192/api/v2/index_show/data/1%' and created_at< '2019-09-30 17:31'  order by created_at desc;